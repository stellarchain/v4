...
export default function TransactionMobileView({ transaction, operations, effects, accountLabels = {}, loading = false }: TransactionMobileViewProps) {
  ...
  }, [loading, transaction.envelope_xdr]);
  ...
  useEffect(() => {
    if (!isContractCall || (activeTab !== 'details' && activeTab !== 'trace') || isDecodingXdr) {
      return;
    }
  ...
  useEffect(() => {
    if (!isContractCall || (activeTab !== 'details' && activeTab !== 'trace') || isDecodingXdr || xdrFetchAttempted || fetchedXdr) {
      return;
    }
      ...
      <main className="px-4 pt-4 max-w-lg mx-auto w-full">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4 text-xs font-medium text-[var(--text-tertiary)]">
          <div className="flex items-center gap-1">
            ...
            <span>{loading ? <InlineSkeleton width="w-24" /> : formatDate(transaction.created_at)}</span>
          </div>
          {loading ? (
            <InlineSkeleton width="w-20" height="h-6" />
          ) : (
          ...
          >
            {loading ? <InlineSkeleton width="w-16" /> : `#${transaction.hash.slice(0, 4)}...${transaction.hash.slice(-3)}`}
            ...
            <div className={`${containers.cardCompact} p-4 mb-4`}>
              ...
              <div className="mt-4 pt-4 border-t border-[var(--border-default)]/50">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[11px] font-bold text-[var(--text-secondary)]">{loading ? <InlineSkeleton width="w-16" /> : `${feeXLM} XLM`}</span>
                </div>
              </div>
            </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        {isSwap ? (
                          ...
                          <span className="flex items-center gap-1">
                            <Link href={sourceIsContract ? `/contract/${displaySourceAccount}` : `/account/${displaySourceAccount}`} className={`text-base font-bold tracking-tight hover:opacity-80 transition-opacity ${!sourceIsContract && accountLabels[displaySourceAccount]?.name ? '' : 'font-mono'}`} style={{ color: primaryColor }}>
                              {!sourceIsContract ? (accountLabels[displaySourceAccount]?.name || shortenAddress(displaySourceAccount)) : shortenAddress(displaySourceAccount)}
                            </Link>
                            {!sourceIsContract && accountLabels[displaySourceAccount]?.verified && (
                              <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="#1D9BF0">
                                <path d="M22.5 12.5c0-1.58-.875-2.95-2.148-3.6.154-.435.238-.905.238-1.4 0-2.21-1.71-3.998-3.818-3.998-.47 0-.92.084-1.336.25C14.818 2.415 13.51 1.5 12 1.5s-2.816.917-3.437 2.25c-.415-.165-.866-.25-1.336-.25-2.11 0-3.818 1.79-3.818 4 0 .494.083.964.237 1.4-1.272.65-2.147 2.018-2.147 3.6 0 1.495.782 2.798 1.942 3.486-.02.17-.032.34-.032.514 0 2.21 1.708 4 3.818 4 .47 0 .92-.086 1.335-.25.62 1.334 1.926 2.25 3.437 2.25 1.512 0 2.818-.916 3.437-2.25.415.163.865.248 1.336.248 2.11 0 3.818-1.79 3.818-4 0-.174-.012-.344-.033-.513 1.158-.687 1.943-1.99 1.943-3.484zm-6.616-3.334l-4.334 6.5c-.145.217-.382.334-.625.334-.143 0-.288-.04-.416-.126l-.115-.094-2.415-2.415c-.293-.293-.293-.768 0-1.06s.768-.294 1.06 0l1.77 1.767 3.825-5.74c.23-.345.696-.436 1.04-.207.346.23.44.696.21 1.04z" />
                              </svg>
                            )}
                            {!sourceIsContract && accountLabels[displaySourceAccount]?.name && !accountLabels[displaySourceAccount]?.verified && (
                      ...
                      <div>
                        ...
                        {isSwap ? (
                          ...
                          <span className="flex items-center">
                            <Link href={destinationIsContract ? `/contract/${displayDestinationAccount}` : `/account/${displayDestinationAccount}`} className="text-base font-bold font-mono tracking-tight hover:opacity-80 transition-opacity" style={{ color: primaryColor }}>
                              {!destinationIsContract ? (accountLabels[displayDestinationAccount]?.name || shortenAddress(displayDestinationAccount)) : shortenAddress(displayDestinationAccount)}
                            </Link>
              ...
              <div className="bg-[var(--bg-secondary)] rounded-2xl shadow-sm border border-[var(--border-default)] overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  {!isDecodingXdr && decodedMeta && decodedMeta.success && (() => {
                    // Pre-process trace: merge fn_call + fn_return pairs, filter out standalone fn_return
                    const processedTrace: (typeof decodedMeta.invocationTrace[0] & { matchedReturn?: typeof decodedMeta.returnValue })[] = [];
                    const pendingCalls: Map<number, (typeof processedTrace[0])[]> = new Map();

                    for (const item of decodedMeta.invocationTrace) {
                      if (item.type === 'fn_call') {
                        const entry = { ...item };
                        processedTrace.push(entry);
                    ...
                    return (
                      <>
                                ...
                                <span className="text-[var(--text-secondary)]">
                                  {processedTrace[0].args.map((a, i) => (
                                    <span key={i}>
                                      ...
                                      <span className="text-[var(--primary-blue)]">{a.display}</span>
                                    </span>
                                  ))}
                                </span>
                              ) : (
                        ...
                        {processedTrace.length > 0 && (
                          <div className="space-y-1.5">
                            {processedTrace.slice(1).map((call, idx) => (
                              <div
                                key={idx}
                                className="flex items-start gap-2.5"
                                style={{ marginLeft: `${Math.min(call.depth, 6) * 16}px` }}
                              >
                                {/* Tree connector line */}
                                <div className="flex flex-col items-center shrink-0 mt-0.5">
                                  <div className={`w-5 h-5 rounded-full flex items-center justify-center ${call.type === 'fn_call' ? 'bg-[var(--primary-blue)]/15' : 'bg-[var(--warning)]/15'
                                    }`}>
                                    {call.type === 'fn_call' ? (
                                ...
                                <div className="flex-1 min-w-0">
                                  {call.type === 'fn_call' && (
                                    <div className="text-[11px] font-mono leading-relaxed">
                                      ...
                                      {call.functionName && (
                                        <>
                                          <span className="text-[var(--text-muted)]"> </span>
                                          <span className="text-[var(--accent)] font-semibold">{call.functionName}</span>
                                          <span className="text-[var(--text-tertiary)]">(</span>
                                  ...
                                  {call.type === 'event' && (
                                    <div className="text-[11px] font-mono leading-relaxed">
                                      ...
                                      {call.args && call.args.length > 0 && (
                                        <span className="text-[var(--text-secondary)]">
                                          ...
                                          {call.args.map((a, i) => (
                                            <span key={i}>
                                              {i > 0 && ', '}
                                              <span className="text-[var(--warning)]">{a.display}</span>
                                            </span>
                                          ))}
                                          {']'}
                                        </span>
                                      ...
                                      {call.returnValue && (
                                        <span className="text-[var(--text-secondary)]"> data: <span className="text-[var(--warning)]">{call.returnValue.display}</span></span>
                                      )}
                                    </div>
                                  )}
                                </div>
                                  ...
                                  <div key={idx} className={`${colors.bg} rounded-lg px-3 py-2 border ${colors.border}`}>
                                    ...
                                    {(!event.decodedParams || Object.keys(event.decodedParams).length === 0) && (
                                      <>
                                        ...
                                        {event.data && (
                                          <div className={`text-[10px] font-mono ${colors.icon} break-all`}>
                                            {event.data.display}
                                          </div>
                                        )}
                                      </>
